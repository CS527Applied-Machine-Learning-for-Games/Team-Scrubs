...
from utils.image_reading import read_image
from utils.imageutils import pad_resize
...
class ImageDataGenerator(object):
    ...
    def flow_from_list(self, list_file,
                       directory=None,
                       target_size=(256, 256), hw_only=False,
...
class ListIterator(Iterator):
    """Iterator capable of reading images from a list of files.

    # Arguments
        list_file: a file that contains the list of files and the corresponding classes.
        directory: parent directory if the file paths in list_file are relative paths.
        image_data_generator: Instance of `ImageDataGenerator`
            to use for random transformations and normalization.
        target_size: tuple of integers, dimensions to resize input images to.
        hw_only: if True, only zero-pad the height and width. 3D image only.
        color_mode: One of `"rgb"`, `"grayscale"`. Color mode to read images.
        classes: Optional list of strings, names of subdirectories
            containing images from each class (e.g. `["dogs", "cats"]`).
            It will be computed automatically if not set.
    ...
    def __init__(self, list_file, image_data_generator, directory=None,
                 target_size=(256, 256), hw_only=False, num_input_channels=1,
                 class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
                 data_format=None):
        if data_format is None:
        ...
        if self.data_format == 'channels_last':
            self.image_shape = self.target_size + (self.num_input_channels,)
        else:
            self.image_shape = (self.num_input_channels,) + self.target_size
        ...
        ext_list = ['.nii', '.nii.gz', '.dcm', '.jpg', '.jpeg', '.png', '.tif', '.tiff', '.bmp', '.npy', '.npz']
        ...
        for l in lines:
            fname, label = l.split()
            if any(fname.lower().endswith(e) for e in ext_list):
                ...
                if self.class_mode == 'binary':  # i.e. 011010
                    ...
                    self.classes.append(binary)
                else:
                    self.classes.append(int(label))
                self.samples += 1
            else:
                raise NameError('Invalid image format: (%s).' % fname)

        self.classes = np.array(self.classes)
        self.num_classes = int(np.max(self.classes)) + 1 if self.class_mode == 'categorical' else None
        super(ListIterator, self).__init__(self.samples, batch_size, shuffle, seed)

    def _get_batches_of_transformed_samples(self, index_array):
        batch_x = np.zeros((len(index_array),) + self.image_shape, dtype=K.floatx())
        # build batch of image data
        for i, j in enumerate(index_array):
            gray_scale = True if self.num_input_channels == 1 else False
            image = read_image(self.filenames[j], gray_scale=gray_scale)
            image = pad_resize(image, self.target_size, hw_only=self.hw_only, make_copy=False)

            if image.ndim == len(self.target_size):
                if self.data_format == 'channels_last':
            ...
            x = self.image_data_generator.random_transform(image)
            x = self.image_data_generator.standardize(x)

            axis = -1 if self.data_format == 'channels_last' else 0
            if x.shape[axis] != self.num_input_channels:
                x = x.repeat(self.num_input_channels, axis=axis)
