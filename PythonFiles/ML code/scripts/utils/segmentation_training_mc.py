...
def segmentation_training(
    ...
    #     # print >> f, ''
    ncluster=aug_args.get("ncluster")
    nclass=aug_args.get("nclass")
    ...
    print(input_arrays.get_valid_num_batches(batch_size),input_arrays.get_test_num_batches(batch_size))

    # Maximum validation batches when no shuffle
    max_valid_batches = input_arrays.get_valid_num_batches(batch_size)
    max_test_batches = input_arrays.get_test_num_batches(batch_size)
    ...
    for epoch in range(n_epochs):
        ...
        for image, label in input_arrays.get_train_flow(batch_size, aug_args, True):
            ...
            label = to_categorical(label, np.sum(ncluster))
            # print("ncluster: ", ncluster)
        ...
        seg00=np.zeros((sz[0],sz[1],sz[2]),np.float32)
        for image, label in input_arrays.get_valid_flow(batch_size, valid_shuffle):
            ...
            seg, seg_mc = get_segmentation(image,nclass,ncluster)
            # seg0[i,:,:]=seg
            # seg00[i,:,:]=seg_mc
            i=i+1
        ...
        #     best_model_updated=1
            # print('seg_all.shape: ',seg0)
            # itkimage = sitk.GetImageFromArray(seg0)
            # # itkimage.SetSpacing(spacing)
            # # itkimage.SetOrigin(origin)
            # sitk.WriteImage(itkimage, 'best_valid_seg.nii.gz', True)
            # print('save seg to best_valid_seg.nii.gz')
        ...
        label_all = []
        n_batches = 0
        sz=input_arrays.image_test.shape
        print('in train')
        print(sz)
        i=0
        seg1=np.zeros((sz[0],sz[1],sz[2]),np.float32)
        seg2=np.zeros((sz[0],sz[1],sz[2]),np.float32)
        print('check batch sizes')
        print(input_arrays.get_test_num_batches(batch_size))
        ...
        if (dice_epoch_mean > max_dice):
            ...
            itkimage = sitk.GetImageFromArray(seg00)
            sitk.WriteImage(itkimage, output_model_path + '_best_valid_seg_mc.nii.gz', True)
            print('save seg to '+output_model_path +'_best_valid_seg_mc.nii.gz')
            ...
            itkimage = sitk.GetImageFromArray(seg2)
            sitk.WriteImage(itkimage, output_model_path + '_best_test_seg_mc.nii.gz', True)
            print('save seg to ' + output_model_path + '_best_test_seg_mc.nii.gz')
