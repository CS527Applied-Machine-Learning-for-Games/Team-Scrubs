...
data_table = pd.read_csv('../data/mixlabel/train_list', delimiter=' ')
train_img_files = pd.read_csv('../data/mixlabel/train_list', delimiter=' ')['file'].values
train_seg_files = pd.read_csv('../data/mixlabel/train_label_list', delimiter=' ')['file'].values
test_img_files = pd.read_csv('../data/mixlabel/test_list', delimiter=' ')['file'].values
test_seg_files = pd.read_csv('../data/mixlabel/test_label_list', delimiter=' ')['file'].values
valid_img_files = pd.read_csv('../data/mixlabel/valid_list', delimiter=' ')['file'].values
valid_seg_files = pd.read_csv('../data/mixlabel/valid_label_list', delimiter=' ')['file'].values
# unlabeled_img_files = pd.read_csv('2D/mixlabel/unlabeled_list', delimiter=' ')['file'].values
train_IDs = pd.read_csv('../data/mixlabel/train_IDs', delimiter=' ')['ID'].values
train_flags = pd.read_csv('../data/mixlabel/train_IDs', delimiter=' ')['flag'].values
valid_IDs = pd.read_csv('../data/mixlabel/valid_IDs', delimiter=' ')['ID'].values
test_IDs = pd.read_csv('../data/mixlabel/test_IDs', delimiter=' ')['ID'].values
...
def generator_train_from_IDlist(train_list, train_flags, view='x', angle=0.0, scale=0):
  ...
  while 1:
    ...
    segfn = '../data/Label_3D/' + id + '/' + view + '/' + id + '_labels_2D_' + view + '_' + str(loc) + '.nii.gz'
    seg = np.squeeze(sitk.GetArrayFromImage(sitk.ReadImage(segfn)))
    while np.max(seg)==0:
        loc = random.randint(0, 511)
        segfn = '../data/Label_3D/' + id + '/' + view + '/' + id + '_labels_2D_' + view + '_' + str(
                loc) + '.nii.gz'
        seg = np.squeeze(sitk.GetArrayFromImage(sitk.ReadImage(segfn)))

    imgfn = '../data/Image_3D/' + id + '/' + view + '/' + id + '_2D_' + view + '_' + str(loc) + '.nii.gz'
    segfn = '../data/Label_3D/' + id + '/' + view + '/' + id + '_labels_2D_' + view + '_' + str(loc) + '.nii.gz'
...
def generator_volume(id, view, nlabel):
    ...
    prefix = '../data/Image_3D/' + id + '/' + view + '/' + id + '_2D_' + view
    seg_prefix = '../data/Label_3D/' + id + '/' + view + '/' + id + '_labels_2D_' + view
    flag=1
    length=0
    while flag>0:
...
def generator_test(id):
    prefix = '../data/Image_3D/'+id
    seg_prefix = '../data/Label_3D/'+id
    flag=1
    length=0
    while flag>0:
        fn=prefix+'_'+str(length)+'.nii.gz'
...
for n_round in range(N_epoch):
    ...
    for i in range(N_per_epoch):
        ...
        print("Saving image")
        sitk.WriteImage(sitk.GetImageFromArray(seg_supervised), output_folder + "/image.nii.gz")
